...
"""
Test SDDMMNet (Sampled Dense-Dense Matrix Multiplication).
"""

import torch
from spardial.backend import spardial_jit
from spardial.models.kernels import MMNet, SDDMMNet


def print_sparse_coo(res):
    print(res[0])
    print(res[1])
    print(res[2])
    print(res[3])
...
def main():
    ...
    # Construct very sparse matrix (COO format)
    ...
    print("pytorch")
    print(mmnet(A, B))
    print(sddmmnet(S, A, B))

    print("spardial")
    print(spardial_jit(mmnet, A, B))
    print_sparse_coo(spardial_jit(sddmmnet, S, A, B))
